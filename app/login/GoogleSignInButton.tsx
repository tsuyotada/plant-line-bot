...
export function GoogleSignInButton() {
  ...
  async function handleGoogleSignIn() {
    if (isLineInAppBrowser()) {
      // Google OAuth returns 403 disallowed_useragent in LINE WebView.
      // Send the user to the relay page so they can open in an external browser.
      window.location.href = "/open-in-browser?next=/login";
      return;
    }
  ...
  return (
    <>
      ...
      {inLine && (
        <p style={{ fontSize: 11, color: "#9ca3af", margin: "0 0 14px", lineHeight: 1.65, textAlign: "center" }}>
          LINEブラウザではGoogleログインは使えません。上の「LINEでログイン」をお使いください。
        </p>
      )}
    </>
  );
